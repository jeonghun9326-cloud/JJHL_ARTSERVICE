...
3. **노래방 자막 제작기** (`/song/`) — 보컬+가사로 노래방 스타일 자막 제작, SRT/ASS 자막 내보내기 및 mp4/webm 영상 내보내기 (React/Vite, `song-app/`, 전부 브라우저 안에서 처리 — 아래 "알려진 제한사항" 참고)
...
- **노래방 자막 제작기(`/song/`)에는 "실제 음성인식 정렬(Forced Alignment)"이 없습니다.** 로컬 Python 서버(`song-app/alignment-server`, FastAPI+torch)가 있어야 동작하는데, 그 서버는 항상 "방문자 자신의 PC"에서 실행돼야 해서 배포된 사이트에서는 서버를 실행한 사람 본인 외에는 아무도 쓸 수 없어 UI에서 제거했습니다(자세한 이유는 아래 "오디오 클리너" 항목과 동일). 파형 에너지 기반 자동 정렬, 수동 타이밍 입력, SRT/ASS 자막 내보내기, 브라우저 노래방 미리보기는 전부 서버 없이 100% 동작합니다. `song-app/alignment-server`의 코드 자체는 남아 있으니 로컬에서 개인적으로 실행하며 개발/실험할 수 있고, 여러 사용자가 함께 쓰게 하려면 그 서버를 공개적으로(항상 켜져 있는 호스트에) 올린 뒤 프론트엔드를 다시 연결해야 합니다.
- **영상(mp4/webm) 내보내기는 서버 없이 브라우저 자체 녹화 방식으로 동작합니다.** 예전에는 alignment-server가 ffmpeg으로 빠르게 렌더링해줬지만(로컬 전용이라 배포 사이트에서는 못 씀), 지금은 "노래방 미리보기" 화면을 캔버스에 그대로 그리면서 `canvas.captureStream()` + `MediaRecorder`로 실시간 녹화합니다. 그래서 곡 길이만큼 시간이 걸리고(예: 3분 곡이면 녹화도 3분), mp4(H.264)를 지원하지 않는 브라우저에서는 자동으로 webm으로 저장됩니다. 최신 Chrome/Edge/Safari 권장.
